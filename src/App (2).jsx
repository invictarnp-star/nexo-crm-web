...
import {
  ...
  Link2, ExternalLink, Bot, Send, Sparkles
} from "lucide-react";
...
.nexo-ia-fab {
  ...
  box-shadow: 0 8px 24px rgba(0,0,0,0.35); transition: transform .15s ease;
}
.nexo-ia-fab:hover { transform: scale(1.06); }
.nexo-ia-panel {
  ...
  box-shadow: 0 20px 60px rgba(0,0,0,0.45); display: flex; flex-direction: column; overflow: hidden;
  animation: nexoLoginUp .18s ease;
}
.nexo-ia-header {
...
.nexo-ia-header-title { font-size: 13.5px; font-weight: 700; line-height: 1.2; }
.nexo-ia-header-sub { font-size: 10.5px; color: var(--text-faint); }
.nexo-ia-close { margin-left: auto; background: none; border: none; color: var(--text-dim); cursor: pointer; padding: 4px; }
.nexo-ia-messages { flex: 1; overflow-y: auto; padding: 14px 14px 6px; display: flex; flex-direction: column; gap: 10px; }
.nexo-ia-empty { color: var(--text-faint); font-size: 12.5px; text-align: center; margin-top: 24px; line-height: 1.6; }
.nexo-ia-msg { max-width: 86%; padding: 9px 12px; border-radius: 12px; font-size: 12.8px; line-height: 1.5; white-space: pre-wrap; word-break: break-word; }
.nexo-ia-msg.user { align-self: flex-end; background: var(--accent); color: #fff; border-bottom-right-radius: 3px; }
.nexo-ia-msg.assistente { align-self: flex-start; background: var(--surface-3); color: var(--text); border-bottom-left-radius: 3px; }
.nexo-ia-msg.erro { align-self: flex-start; background: var(--danger-soft); color: var(--danger); border-bottom-left-radius: 3px; }
.nexo-ia-input-row { display: flex; gap: 8px; padding: 12px; border-top: 1px solid var(--border-soft); flex-shrink: 0; }
.nexo-ia-input-row textarea {
  flex: 1; resize: none; background: var(--surface-2); border: 1px solid var(--border); border-radius: 10px;
  color: var(--text); font-family: inherit; font-size: 12.8px; padding: 9px 11px; min-height: 38px; max-height: 90px;
}
.nexo-ia-input-row textarea:focus { outline: none; border-color: var(--accent); }
.nexo-ia-send-btn {
  width: 38px; height: 38px; border-radius: 10px; border: none; cursor: pointer; flex-shrink: 0;
  background: var(--accent); color: #fff; display: flex; align-items: center; justify-content: center;
}
.nexo-ia-send-btn:disabled { opacity: .5; cursor: default; }
@media (max-width: 480px) {
  .nexo-ia-panel { right: 16px; left: 16px; width: auto; bottom: 84px; height: calc(100vh - 160px); }
  .nexo-ia-fab { right: 16px; bottom: 16px; }
}
...
// Monta um resumo dos dados do sistema para enviar como contexto ao
// assistente de IA. Propositalmente NÃO inclui dados pessoais sensíveis
// do cliente (CPF, telefone, WhatsApp, e-mail, endereço, CEP, data de
// nascimento, CNH) — só o que o assistente precisa para responder sobre
// clientes, veículos e boletos (nome, veículo, valores, vencimentos,
// status de pagamento). O texto final é limitado em tamanho.
function montarContextoParaIA(db) {
  const LIMITE = 11000;
  const linhas = [];

  linhas.push(`Resumo do sistema (${new Date().toLocaleDateString("pt-BR")}):`);
  linhas.push(`- ${db.clientes.length} cliente(s) cadastrado(s), ${db.veiculos.length} veículo(s), ${db.boletos.length} boleto(s).`);

  const veiculosPorCliente = {};
  for (const v of db.veiculos) {
    (veiculosPorCliente[v.clienteId] = veiculosPorCliente[v.clienteId] || []).push(v);
  }
  const boletosPorCliente = {};
  for (const b of db.boletos) {
    (boletosPorCliente[b.clienteId] = boletosPorCliente[b.clienteId] || []).push(b);
  }

  const hoje = new Date().toISOString().slice(0, 10);
  linhas.push("\nClientes (nome, status, veículos, boletos em aberto/atrasados):");
  for (const c of db.clientes) {
    if (linhas.join("\n").length > LIMITE) { linhas.push("… (lista truncada por tamanho)"); break; }
    const veic = (veiculosPorCliente[c.id] || [])
      .map((v) => `${v.marca} ${v.modelo} (placa ${v.placa || "?"})`)
      .join("; ") || "sem veículo cadastrado";
    const boletosCliente = boletosPorCliente[c.id] || [];
    const abertos = boletosCliente.filter((b) => !b.dataPagamento);
    const atrasados = abertos.filter((b) => b.dataVencimento && b.dataVencimento < hoje);
    const resumoBoletos =
      boletosCliente.length === 0
        ? "sem boletos"
        : `${boletosCliente.length} boleto(s), ${abertos.length} em aberto${atrasados.length ? `, ${atrasados.length} atrasado(s)` : ""}`;
    linhas.push(`- ${c.nome} [${c.status}] — ${veic} — ${resumoBoletos}`);
  }

  return linhas.join("\n").slice(0, LIMITE);
}

function AssistenteWidget({ db }) {
  const [aberto, setAberto] = useState(false);
  const [pergunta, setPergunta] = useState("");
  const [mensagens, setMensagens] = useState([]); // { papel: "user"|"assistente"|"erro", texto }
  ...
  async function enviarPergunta() {
    ...
    const novoHistorico = [...mensagens, { papel: "user", texto }];
    setMensagens(novoHistorico);
    setPergunta("");
    setEnviando(true);
    try {
      const contexto = montarContextoParaIA(db);
      ...
      const data = await resp.json();
      if (!resp.ok) {
        setMensagens((prev) => [...prev, { papel: "erro", texto: data.erro || "Não foi possível falar com o assistente." }]);
      } else {
        setMensagens((prev) => [...prev, { papel: "assistente", texto: data.resposta }]);
      }
    } catch (e) {
      setMensagens((prev) => [...prev, { papel: "erro", texto: "Falha de conexão com o assistente. Tente novamente." }]);
    } finally {
      setEnviando(false);
    }
  }
  ...
  return (
    <>
      {aberto && (
        <div className="nexo-ia-panel">
          <div className="nexo-ia-header">
            <div className="nexo-ia-header-mark"><Sparkles size={15} /></div>
            <div>
              <div className="nexo-ia-header-title">Assistente Nexo</div>
              <div className="nexo-ia-header-sub">IA (Gemini) · dados sensíveis não são enviados</div>
            </div>
            <button className="nexo-ia-close" onClick={() => setAberto(false)}><X size={16} /></button>
          </div>
          <div className="nexo-ia-messages">
            {mensagens.length === 0 && (
              <div className="nexo-ia-empty">
                Pergunte algo sobre seus clientes, veículos ou boletos.
                <br />Ex.: "quais boletos estão atrasados?"
              </div>
            )}
            {mensagens.map((m, i) => (
              <div key={i} className={`nexo-ia-msg ${m.papel}`}>{m.texto}</div>
            ))}
            {enviando && <div className="nexo-ia-msg assistente">Digitando…</div>}
            <div ref={fimRef} />
          </div>
          <div className="nexo-ia-input-row">
            <textarea
              value={pergunta}
              ...
              disabled={enviando}
            />
            <button className="nexo-ia-send-btn" onClick={enviarPergunta} disabled={enviando || !pergunta.trim()}>
              <Send size={16} />
            </button>
          </div>
        </div>
      )}
      <button className="nexo-ia-fab" onClick={() => setAberto((v) => !v)} title="Assistente de IA">
        <Bot size={24} />
      </button>
    </>
  );
...
export default function App() {
  ...
  return (
    <div className="nexo">
      ...
      {!loading && !loadError && <AssistenteWidget db={db} />}
    </div>
  );
}
